...
public class RenderKakejiku extends Render {
    ...
    public void renderKakejiku(EntityKakejiku par1EntityKakejiku, double par2, double par4, double par6, float par8, float par9)
    {
        ...
        this.bindEntityTexture(par1EntityKakejiku);
    ...
    private void func_77010_a(EntityKakejiku par1EntityKakejiku, int par2, int par3, int par4, int par5)
    {
        ...
        for (int i1 = 0; i1 < par2 / 16; ++i1)
        {
            for (int j1 = 0; j1 < par3 / 16; ++j1)
            {
                ...
                float f18 = f1 + (j1 * 16);
                this.func_77008_a(par1EntityKakejiku, (f15 + f16) / 2.0F, (f17 + f18) / 2.0F);
    ...
    private void func_77008_a(EntityKakejiku par1EntityKakejiku, float par2, float par3)
    {
        ...
        int j = MathHelper.floor_double(par1EntityKakejiku.posY + (par3 / 16.0F));
        int k = MathHelper.floor_double(par1EntityKakejiku.posZ);

        if (par1EntityKakejiku.hangingDirection == 2)
        {
            i = MathHelper.floor_double(par1EntityKakejiku.posX + (par2 / 16.0F));
        }

        if (par1EntityKakejiku.hangingDirection == 1)
        {
            k = MathHelper.floor_double(par1EntityKakejiku.posZ - (par2 / 16.0F));
        }

        if (par1EntityKakejiku.hangingDirection == 0)
        {
            i = MathHelper.floor_double(par1EntityKakejiku.posX - (par2 / 16.0F));
        }

        if (par1EntityKakejiku.hangingDirection == 3)
        {
            k = MathHelper.floor_double(par1EntityKakejiku.posZ + (par2 / 16.0F));
        }
